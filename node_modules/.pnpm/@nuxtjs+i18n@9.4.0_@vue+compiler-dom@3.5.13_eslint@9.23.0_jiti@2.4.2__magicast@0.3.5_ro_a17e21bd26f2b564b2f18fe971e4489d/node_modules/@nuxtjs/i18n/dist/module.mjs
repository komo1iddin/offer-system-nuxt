import { tryResolveModule, useNuxt, resolvePath, useLogger, addTemplate, tryUseNuxt, addBuildPlugin, addWebpackPlugin, addRspackPlugin, extendViteConfig, addServerTemplate, addServerImports, addServerPlugin, createResolver, addPlugin, addVitePlugin, useNitro, addTypeTemplate, addComponent, addImports, defineNuxtModule } from '@nuxt/kit';
...
import { genArrayFromRaw, genObjectFromRaw, genObjectFromValues, genString, genSafeVariableName, genDynamicImport, genImport } from 'knitwork';
...
function resolveLocales(srcDir, locales, buildDir) {
  ...
  for (const locale of locales) {
    const resolved = Object.assign({ meta: [] }, locale);
...
async function extendBundler(ctx, nuxt) {
  ...
  const localeIncludePaths = localePaths.length ? localePaths : void 0;
...
function prepareOptions({ debug, logger, options }, nuxt) {
  ...
  if (!nuxt.options?._prepare && !nuxt.options?.test && nuxt.options.i18n?.bundle?.optimizeTranslationDirective == null) {
    logger.warn(
...
function generateLoaderOptions(ctx, nuxt) {
  ...
  function generateLocaleImports(meta) {
...
function prepareAutoImports({ debug, resolver }, nuxt) {
